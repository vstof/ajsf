...
export function isInputRequired(schema, schemaPointer: string): boolean {
  ...
  if (Array.isArray(listPointerArray)) {
    ...
    if (parentSchema.type === 'array') {
      return (
        hasOwn(parentSchema, 'minItems') && isNumber(keyName) && +parentSchema.minItems > +keyName
      );
    }
  }
  return false;
  ...
  if (Array.isArray(oneOf) && oneOf.every((item) => item.title)) {
    ...
    if (
      ...
      if (
        ...
        newTitleMap.some(
          (title, index) =>
            index && hasOwn(title, 'group') && title.group === newTitleMap[index - 1].group,
        )
      ) {
    ...
    (subSchema, subPointer) => {
      if (isObject(subSchema)) {
        ...
        if (isString(subSchema.$ref)) {
          ...
          if (
            refPointer.length &&
            usedPointers.every((ptr) => !JsonPointer.isSubPointer(refPointer, ptr, true))
          ) {
